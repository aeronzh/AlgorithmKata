...
public class Solution {
    ...
    private boolean isValidIPv4(String ip) {
        ...
        String[] tokens = ip.split("\\."); // "." in regex is a metacharacter, it is used to match any character. "\" is escape
    ...
    private boolean isValidIPv4Token(String token) {
        ...
        if (token.startsWith("0") && token.length() == 1) return true;
    ...
    //     try {
    //         int num = Integer.parseInt(token); // parseInt ignores minus sign

    //         if (num < 0 || num > 255) return false;
    //         if (num == 0 && token.length() > 1) return false;
