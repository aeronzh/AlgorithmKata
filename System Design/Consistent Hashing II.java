...
public class Solution {
    ...
    int n, k;
    Set<Integer> ids; // machine ids
    Map<Integer, List<Integer>> machines; // <machine_id, List<machine_pos>>
    ...
    public int getMachineIdByHashCode(int hashcode) {
        // Write your code here
        int min_distance = n + 1; // n intervals
        int machine_id = 0;
        for (Map.Entry<Integer, List<Integer>> machine : machines.entrySet()) {
            int key = machine.getKey();
            List<Integer> random_nums = machine.getValue();
            for (int num : random_nums) {
                int distance = num - hashcode;
                if (distance < 0) {
                    // anti-clockwise
                    distance += n;
                }
