...
public class Solution {
    ...
    public List<List<Integer>> consistentHashing(int n) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            int index = 0; // if all intervals are equal, split the first interval
            for (int j = 1; j < result.size(); j++) {
                if (result.get(j).get(1) - result.get(j).get(0) > 
                    result.get(j - 1).get(1) - result.get(j - 1).get(0)) {
                        index = j;
                        break;
                    }
            }
            ...
            result.get(index).set(1, x + (y - x) / 2);
            
            machine.add(x + (y - x) / 2 + 1);
            machine.add(y);
            machine.add(i + 1);
            result.add(machine);
        }
